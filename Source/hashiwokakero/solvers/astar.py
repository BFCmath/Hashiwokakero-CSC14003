...
class AStarFCSolver:
    """
    A* solver with Forward Checking.
    
    Kế thừa đầy đủ các tối ưu từ AStarSolver:
    - Pre-compute crossing conflicts
    - Sparse Signature  
    - Tie-breaking (f, h, counter)
    - MRV heuristic
    - Quick feasibility checks
    - Fast crossing check
    
    Thêm Forward Checking để prune sớm các nhánh không khả thi.
    CONSERVATIVE: Chỉ prune khi CHẮC CHẮN không thể đạt được mục tiêu.
    """
    ...
    def solve(self, initial: PuzzleState) -> SearchResult:
        ...
        # Pre-compute crossing conflicts (giống A* thuần)
        conflicts: Dict[int, List[int]] = {cid: [] for cid in grid.corridors}
        corridor_items = list(grid.corridors.values())
        for i in range(len(corridor_items)):
            c1 = corridor_items[i]
        ...
        # OPTIMIZATION 1: Sparse Signature (giống A* thuần)
        def signature(bc: Dict[int, int]) -> Tuple[Tuple[int, int], ...]:
            return tuple(sorted(bc.items()))
        ...
        # OPTIMIZATION 2: Tie-breaking (giống A* thuần)
        # Heap entries: (f, h, counter, g, sig, state)
        init_h = self.heuristic(initial)
        open_heap: List[Tuple[float, float, int, int, Tuple[Tuple[int, int], ...], PuzzleState]] = []
        heapq.heappush(open_heap, (init_h, init_h, counter, 0, init_sig, initial))
        ...
        while open_heap:
            ...
            # MRV: pick island with smallest positive remaining degree (giống A* thuần)
            best_island = None
            best_rem = float("inf")
            for isl in islands:
                rem = current.remaining_degree(isl)
            ...
            for corridor in grid.corridors_incident_to(best_island.identifier):
                ...
                # Quick feasibility checks (giống A* thuần)
                isl_a = grid.islands[corridor.island_a]
                isl_b = grid.islands[corridor.island_b]
                if current.remaining_degree(isl_a) < new_val - cur_val:
                    continue
                ...
                # Fast crossing check (giống A* thuần)
                if cur_val == 0:
                    is_blocked = False
                    for conflict_cid in conflicts[corridor.identifier]:
                        if current.bridge_counts.get(conflict_cid, 0) > 0:
                ...
                # ===== FORWARD CHECKING =====
                # Kiểm tra khả năng đạt được mục tiêu
                # CONSERVATIVE: Chỉ prune khi CHẮC CHẮN không thể đạt được
                if not self._forward_check(next_state, grid, conflicts):
                    continue
                # ============================
    ...
    def _forward_check(self, state: PuzzleState, grid, conflicts: Dict[int, List[int]]) -> bool:
        """
        Forward Checking: Kiểm tra xem state hiện tại có thể dẫn đến lời giải không.
        
        CONSERVATIVE: Chỉ return False khi CHẮC CHẮN không thể đạt được mục tiêu.
        
        Logic:
        - Với mỗi đảo chưa thỏa mãn, tính max số cầu có thể thêm
        - Nếu max < remaining → không thể đạt target → prune
        
        Returns:
            True: Có thể tiếp tục (có khả năng đạt được mục tiêu)
            False: Chắc chắn không thể đạt được mục tiêu
        """
        for island in grid.islands.values():
            remaining = state.remaining_degree(island)
            
            # Đã vượt quá target → không hợp lệ
            if remaining < 0:
                return False
            
            # Đã thỏa mãn → OK, không cần kiểm tra thêm
            if remaining == 0:
                continue
            
            # Tính max số cầu có thể thêm cho đảo này
            max_additional = 0
            for corridor in grid.corridors_incident_to(island.identifier):
                cid = corridor.identifier
                current_bridges = state.corridor_value(cid)
                
                # Corridor đã đầy (2 cầu) → không thể thêm
                if current_bridges >= 2:
                    continue
                
                # Kiểm tra corridor có bị block bởi crossing không
                # CHỈ kiểm tra nếu corridor chưa có cầu nào
                if current_bridges == 0:
                    is_blocked = False
                    for conflict_cid in conflicts[cid]:
                        if state.corridor_value(conflict_cid) > 0:
                            is_blocked = True
                            break
                    if is_blocked:
                        continue
                
                # Corridor này có thể thêm tối đa (2 - current_bridges) cầu
                max_additional += (2 - current_bridges)
            
            # Nếu max có thể thêm < remaining → không thể đạt target
